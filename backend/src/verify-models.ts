...
async function verifyModels() {
  ...
  try {
    ...
    const dummyBacklog = new Backlog({
    ...
    const dummyPart = new BacklogPart({
    ...
    const dummyWorkOrder = new WorkOrder({
      backlogId: dummyBacklog._id,
      woNumber: 'WO-987654',
      targetDate: new Date(),
    ...
    const dummyPhoto = new BacklogPhoto({
      backlogId: dummyBacklog._id,
      photoType: 'BEFORE',
    ...
    const dummyCompletion = new Completion({
      backlogId: dummyBacklog._id,
    ...
    const dummyHistory = new BacklogHistory({
      backlogId: dummyBacklog._id,
      fromStatus: null,
