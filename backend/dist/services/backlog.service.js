...
class BacklogService {
     ...
     */
    async transitionStatus(backlogId, fromStatus, toStatus, actionByUser) {
    ...
    async createBacklog(creatorUser, backlogData) {
        ...
        if (photoUrl) {
            photoUrl = await (0, blob_service_1.uploadBase64ToBlob)(photoUrl, backlog.backlogNo, 'BEFORE');
        }
        // Save photo
        ...
        await this.notificationService.sendNotificationToRole('GL', { site: backlog.site, section: backlog.section }, 'Backlog Baru Menunggu Approval', `Mekanik ${creatorUser.name} membuat backlog baru ${backlog.backlogNo}. Menunggu approval Anda.`);
        return backlog;
    }
    /**
     ...
     */
    async approveBacklog(approverUser, backlogId) {
        ...
        await this.notificationService.sendNotificationToRole('PLANNER', { site: backlog.site, section: backlog.section }, 'Backlog Approved', `Backlog ${backlog.backlogNo} telah disetujui oleh GL ${approverUser.name}. Silakan buat Work Order.`);
     ...
     */
    async rejectBacklog(rejecterUser, backlogId) {
        const backlog = await this.backlogRepository.findById(backlogId);
        if (!backlog) {
            throw new Error('Backlog not found');
        }
     ...
     */
    async startInstallation(mekanikUser, backlogId) {
        const backlog = await this.backlogRepository.findById(backlogId);
        if (!backlog) {
            throw new Error('Backlog not found');
        }
    ...
    async completeBacklog(mekanikUser, backlogId, completionData) {
        const backlog = await this.backlogRepository.findById(backlogId);
        if (!backlog) {
            throw new Error('Backlog not found');
        }
        ...
        if (completionPhotoUrl) {
            completionPhotoUrl = await (0, blob_service_1.uploadBase64ToBlob)(completionPhotoUrl, backlog.backlogNo, 'AFTER');
        }
        ...
        // Notify Planners in the same site and section
        await this.notificationService.sendNotificationToRole('PLANNER', { site: backlog.site, section: backlog.section }, 'Backlog Completed', `Backlog ${backlog.backlogNo} telah selesai dikerjakan oleh Mekanik ${mekanikUser.name}.`);
        await this.notificationService.sendNotificationToRole('GL', { site: backlog.site, section: backlog.section }, 'Backlog Completed', `Backlog ${backlog.backlogNo} telah selesai dikerjakan oleh Mekanik ${mekanikUser.name}.`);
        return updatedBacklog;
    }
    /**
     * Retrieve list of backlogs, scoped strictly based on the user's role and site/section permissions.
     */
    async getBacklogs(user, page = 1, limit = 10) {
        const filter = {};
    ...
    async addPartToBacklog(user, backlogId, partData) {
        const backlog = await this.backlogRepository.findById(backlogId);
        if (!backlog) {
            throw new Error('Backlog not found');
        }
    ...
    async addPhotoToBacklog(user, backlogId, photoData) {
        const backlog = await this.backlogRepository.findById(backlogId);
        if (!backlog) {
            throw new Error('Backlog not found');
        }
        ...
        if (photoUrl) {
            photoUrl = await (0, blob_service_1.uploadBase64ToBlob)(photoUrl, backlog.backlogNo, photoData.photoType || 'ADDITIONAL');
        }
